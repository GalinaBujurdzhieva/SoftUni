...
SELECT e.FirstName + ' ' + e.LastName AS FullName, COUNT(DISTINCT r.Id) AS UsersCount
FROM Employees AS e
...
BEGIN
    ...
    DECLARE @result BIGINT = DATEDIFF(HOUR, @StartDate, @EndDate)
	RETURN @result
END
GO
...
AS
BEGIN
DECLARE @myEmployeeDepartment INT = (SELECT DepartmentId 
                                     FROM Employees
                                     WHERE Id = @EmployeeId)

DECLARE @myReportDepartment INT = (SELECT c.DepartmentId
                                   FROM Reports AS r
								   JOIN Categories AS c ON r.CategoryId = c.Id
								   WHERE r.Id = @ReportId)
BEGIN TRANSACTION
       IF (@myEmployeeDepartment != @myReportDepartment)
       BEGIN
                  ROLLBACK;
